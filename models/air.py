...
def update_database(update_values: List[AirSchema]) -> bool:
    ...
    while True:
        ...
        if retry_counter == MAX_RETRY_COUNT:
            log.error("Was unable to connect to the database.")
            return False
        ...
        try:
            ...
            log.error(
                f"Unable to connect to the database, will retry in {RETRY_SLEEP_COUNT} seconds...")
            retry_counter += 1
    ...
    for model in update_values:
        ...
        cur.execute(f"SELECT * from air WHERE country='{model.country}'")
        result = cur.fetchall()
        ...
        if result_length == 0:
            ...
            if model.co2_gradient and model.no_gradient:
                cur.execute(
                    "INSERT INTO air VALUES(%s,%s,%s,%s,%s)", (
                        model.country,
                        model.co2_gradient,
                        model.co2_offset,
                        model.no_gradient,
                ...
                cur.execute(
                    "INSERT INTO air (country, co2_gradient, co2_offset) VALUES(%s,%s,%s)", (
                        model.country,
                        model.co2_gradient,
                        model.co2_offset))
                conn.commit()
                ...
                cur.execute(
                    "INSERT INTO air (country, no_gradient, no_offset) VALUES(%s,%s,%s)", (
                        model.country,
                        model.co2_gradient,
                        model.co2_offset))
                conn.commit()
                ...
                log.error(
                    "Don't have either product... this shouldn't happen...")

        elif result_length == 1:
            if model.co2_gradient and not model.no_gradient:
                cur.execute(
                    "UPDATE air SET co2_gradient=%s, co2_offset=%s WHERE country=%s", (
                        model.co2_gradient,
                        model.co2_offset,
                        model.country))
                conn.commit()
                ...
                cur.execute(
                    "UPDATE air SET no_gradient=%s, no_offset=%s WHERE country=%s", (
                        model.no_gradient,
                        model.no_offset,
                        model.country))
                conn.commit()
...
def process_dataset(dataset_path: str) -> Union[List[AirSchema], None]:
    ...
    for key in grouped.groups.keys():
        ...
        if nitrogen_oxide_levels and carbon_dioxide_levels:
            ...
            linear_models.append(AirSchema(
                key,
            ...
            log.error(
                "Neither carbon nor nitrogen found, this shouldn't happen...")
            return

    return linear_models


def on_created(file: str) -> None:
