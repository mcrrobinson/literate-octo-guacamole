...
def setup_logging_config(name:str, output_file_name:str) -> logging.Logger:
    ...
    if 'PRODUCTION' in environ:
        logging.basicConfig(
            handlers=[RotatingFileHandler(filename='/logs/{}'.format(output_file_name),
            mode='w', maxBytes=512000, backupCount=4)], level=logging.DEBUG,
            format='%(levelname)s %(asctime)s %(message)s', 
            datefmt='%m/%d/%Y%I:%M:%S %p')
    else:
        logging.basicConfig(
            handlers=[RotatingFileHandler(filename=output_file_name,
            mode='w', maxBytes=512000, backupCount=4)], level=logging.DEBUG,
            format='%(levelname)s %(asctime)s %(message)s', 
            datefmt='%m/%d/%Y%I:%M:%S %p')
