...
try:
    ...
    log.error(
        "Unable to convert RETRY_SLEEP_COUNT to an integer. Reverting to default...", err)


def update_database(update_values: List[HeatSchema]) -> bool:
    ...
    while True:
        ...
        if retry_counter == MAX_RETRY_COUNT:
            log.debug("Was unable to connect to the database.")
            return False
        ...
        try:
            ...
            log.debug(
                f"Unable to connect to the database, \
                    will retry in {RETRY_SLEEP_COUNT} seconds...")
            retry_counter += 1
            sleep(RETRY_SLEEP_COUNT)
    ...
    for model in update_values:
        ...
        cur.execute(f"SELECT * from heat WHERE country='{model.country}'")
        result = cur.fetchall()
        ...
        if result_length == 0:
            ...
            log.debug(f"Inserting country: {model.country} into the database.")
            cur.execute(
                "INSERT INTO heat VALUES(%s,%s,%s,%s,%s,%s,%s)", (
                    model.country,
                    model.min_gradient,
                    model.min_offset,
                    model.avg_gradient,
            ...
            # This shouldn't happen, make an arbitrary fix...
    cur.close()

    log.debug("Successfully inserted Linear Regression Coefficients into the \
        database...")
    return True
...
def process_dataset(dataset_path: str) -> Union[List[HeatSchema], None]:
    ...
    data[date_formatted_column] = data[date_column_name].dt.strftime(
        '%Y%m%d').astype(int)

    # If not all the required information was found, raise error and return.
    if country_column_name is None:
        log.debug(
            "Couldn't locate the country column in the supplied dataset.")
        return

    if date_column_name is None:
        log.debug(
            "Couldn't locate the date column in the supplied dataset.")
        return

    if temp_column_name is None:
        log.debug(
            "Couldn't locate the temperature column in the supplied dataset.")
        return
    ...
    for key in grouped.groups.keys():
        ...
        for year in country_by_year.groups.keys():
            ...
            pandas_data['max'].append(
                np.amax(temperatures_per_country_per_year))

            pandas_data['min'].append(
                np.amin(temperatures_per_country_per_year))

            pandas_data['date'].append(
                min(temperatures_per_country_per_year_date))
        ...
        min_linear_regression = linear_regression(
            pandas_dataframe['date'],
            pandas_dataframe['min'])

        max_linear_regression = linear_regression(
            pandas_dataframe['date'],
            pandas_dataframe['max'])

        linear_models.append(HeatSchema(
            key,
...
def on_created(path: str):
    ...
    result = update_database(linear_regression_models)
    if not result:
...
if __name__ == '__main__':
    log.debug("Starting listener for air models...")

    length_of_arguments = len(sys.argv)
    if length_of_arguments < 2:
        print("Please provide the path to the dataset directory.")
    elif (length_of_arguments > 2):
        print("Too many arguments provided, please provide only the path to the dataset directory.")

    if not os.path.isfile(sys.argv[1]):
        print("Please provide the path to the dataset directory.")
    else:
        on_created(sys.argv[1])
